...
public class TimelineActivity extends TrackedActivity implements AdListener {
	...
	protected void onResume() {
		...
		if (Account.getTwitterAccount(this) == null
				&& Account.getFacebookAccount(this) == null)
			finish();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		long lastAdShow = settings.getLong(Constants.LAST_AD_SHOW, -1);
		long distAdShown=new Date().getTime()-lastAdShow;
		if (lastAdShow == -1 || distAdShown > 300000) {
	...
	private void loadCollumns() {
		int cont = 0;
		LabelRefresListenerList labelListenerSubject = LabelRefresListenerList
				.getInstance();
		for (CollumnConfig config : Facade.getInstance(this).getAllConfig()) {
			...
			if (config.getType().equals(CollumnConfig.FACEBOOK_COLLUMN)) {
				...
				adapter.addView(column);
				labelListenerSubject
						.registerListerner((IOnLabelRefreshListener) column);
	...
	public void onReceiveAd(Ad ad) {
		...
		SharedPreferences settings = getSharedPreferences(Constants.PREFS_NAME,
				0);
		SharedPreferences.Editor editor = settings.edit();
		editor.putLong(Constants.LAST_AD_SHOW, new Date().getTime());
